...
def transcribeNewUploads():
    ...
    if newJobs:
        for job in newJobs:
            ...
            # Video file location
            uploadFile = str(settings.BASE_DIR)+"/media/uploads/" + \
                str(job.filename).split('/')[-1]
            # Audio file location
            filename = str(settings.BASE_DIR) + \
                '/media/uploads/' + str(job.id) + ".mp3"
            ...
            with data:
                ...
                s3.Bucket('subgenstoragebucket').put_object(
                    Key=newLoc, Body=data)
